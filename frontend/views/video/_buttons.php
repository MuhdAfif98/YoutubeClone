...
<a href="<?php echo Url::to(['/video/like', 'id' => $model->video_id]) ?>"
    //* if the current video liked by current user, button primary otherwise secondary
...
<a href="<?php echo Url::to(['/video/dislike', 'id' => $model->video_id]) ?>"
   class="btn btn-sm <?php echo $model->isDislikedBy(Yii::$app->user->id) ? 'btn-outline-danger': 'btn-outline-secondary' ?>"
   data-method="post" data-pjax="1">
